...
import in.gov.ipie.service.user.event.AccountProvisionedPayload;
...
class AccountProvisionedConsumer {

    private static final Logger LOG = LoggerFactory.getLogger(AccountProvisionedConsumer.class);

    private final UserService userService;
    private final ProcessedEventStore processedEventStore;

    AccountProvisionedConsumer(UserService userService, ProcessedEventStore processedEventStore) {
    ...
    void onAccountProvisioned(EventEnvelope<AccountProvisionedPayload> event) {
        IdempotentEventHandler.handle(event.eventId(), processedEventStore, () -> {
            AccountProvisionedPayload payload = event.data();
