...
import in.gov.ipie.service.user.repository.PillarIdpWriteBack;
...
public class PillarLinkServiceImpl implements PillarLinkService {
    ...
    private final PillarIdpWriteBack pillarIdpWriteBack;
    ...
    /**
     * Delegates to {@link PillarIdpTokenClient}: the provider's endpoints, client id and redirect
     * URI are the adapter's to know. What stays here is that the link request's own id is the
     * {@code state} - that is linking policy, and it is what {@code completeLinkCallback} looks
     * the request up by.
     */
    @Override
    public String buildAuthorizationUrl(PillarLinkRequest request) {
        return pillarIdpTokenClient.buildAuthorizationUrl(request.getPillarType(), request.getId());
    }
    ...
    public PillarLink completeLinkCallback(String code, UUID state) {
        ...
        if (claims.sub() != null) {
            try {
                pillarIdpWriteBack.writeIpieId(request.getPillarType(), claims.sub(), request.getUserId());
